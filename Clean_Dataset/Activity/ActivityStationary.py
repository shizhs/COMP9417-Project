...
for file in files:
    ...
    uid = file.replace('activity_', '').split('.')[0]
    step1 = textFile.map(lambda line: ((getWeek(line.split(",")[0]), line.split(",")[1]), [1, {getDay(line.split(",")[0])}]))
    ...
    for v in dic.keys():
        if int(v[1]) == 0:
            outList[int(v[0])] = dic[v][0] * 2.5 // 60 // len(dic[v][1])
    s = uid
    for i in outList[1:]:
...
for t in outputFile:
    s += t[1] + '\n'
with open('./output/activityStationary.csv', 'w') as file:
    file.write(s)
