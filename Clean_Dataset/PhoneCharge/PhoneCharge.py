...
inPath = './input/phonecharge/'
ouPath = './result/'
files = os.listdir(inPath)
outputFile = []

for file in files:
    textFile = sc.textFile(inPath + file).filter(lambda s: not ('start' in s or '' == s))
    uid = file.replace('phonecharge_', '').split('.')[0]
    step1 = textFile.map(lambda line: (getWeek(line.split(",")[0]), [int(line.split(",")[1]) // 60 -
                                                                     int(line.split(",")[0]) // 60, 1]))
...
for t in outputFile:
    s += t[1] + '\n'
with open('./output/phonecharge.csv', 'w') as file:
    file.write(s)
